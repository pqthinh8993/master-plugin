...
class Mastercard_Gateway extends WC_Payment_Gateway {
	...
	const MPGS_API_VERSION = 'version/72';
	const MPGS_API_VERSION_NUM = '72';
	...
	const THREED_V2 = '2';
    const gateway_currency = "VND";
    const gateway_currency_rate = 23365;
	/**
	 * @var string
	 */
	...
	public function process_capture() {
		...
		$result = $this->service->captureTxn(
			...
			time(),
			(float) $order->get_total() * self::gateway_currency_rate,
			self::gateway_currency//$order->get_currency()
		);
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		$result = $this->service->refund(
			...
			(string) time(),
			$amount * self::gateway_currency_rate,
			self::gateway_currency//$order->get_currency()
		);
	...
	protected function process_hosted_session_payment( $three_ds_txn_id = null ) {
		...
		if ( $check_3ds ) {
			...
			$orderData = array(
				'amount'   => (float) $order->get_total() * self::gateway_currency_rate,
				'currency' => self::gateway_currency//$order->get_currency()
			);
	...
	protected function validate_order( $order, $mpgs_order ) {
		if ( self::gateway_currency !== $mpgs_order['currency'] ) {
			throw new Exception( $mpgs_order['currency'].'Currency mismatch' );
		}
        // if ( $order->get_currency() !== $mpgs_order['currency'] ) {
		// 	throw new Exception( 'Currency mismatch' );
		// }

		if ( (float) $order->get_total() * self::gateway_currency_rate !== (float) $mpgs_order['amount'] ) {
